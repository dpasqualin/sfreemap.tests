...
plot_comparison <- function(x, xlabel, limit=NULL, mode='serial', output=NULL) {
    ...
    data <- parse(files[1], types[1], legend[1], limit, mode, FALSE)
    for (i in 2:length(files)) {
        tmp <- parse(files[i], types[i], legend[i], limit, mode, FALSE)
        data <- rbind(data, tmp)
    }
